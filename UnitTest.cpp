#include "pch.h"
#include "../Assignment_Data_01/i190525_DSAss_01.h"
...
{
	string path = "File Path Here."; /*give path of the student dataset file in this string*/
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 17; j++)
		{
			EXPECT_DOUBLE_EQ(Data[i][j], expectedArray[i][j]);
		}
	}
}
TEST(MyBrothersInDsTest, Distance_Test)
{
	...
	distancee = Distance(Predicted_Array, 14, Data, 310, 17);
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			EXPECT_DOUBLE_EQ(distancee[i][j], expectedArray[i][j]);
		}
	}
}
TEST(MyBrothersInDsTest, Sorted_K_Distance_Test)
{
	...
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			EXPECT_DOUBLE_EQ(sortedKDistance[i][j], expectedArray[i][j]);
		}

	}
}
TEST(MyBrothersInDsTest, Grade_Frequence_Test1)
{
...
TEST(MyBrothersInDsTest, Grade_Frequence_Test2)
{
	string str = "A"; //A and B+ are also acceptable. if your output is A+ or B+ on this input then change the value of str accordingly and your test will be passed 
	string grade = Grade_Frequency(sortedKDistance, 2, 2);
	EXPECT_EQ(str, grade);
}
TEST(MyBrothersInDsTest, Grade_Frequence_Test3)
{
	string str = "A"; //A and B+ are also acceptable. if your output is A+ or B+ on this input then change the value of str accordingly and your test will be passed 
	string grade = Grade_Frequency(sortedKDistance, 2, 2);
	EXPECT_EQ(str, grade);
...
TEST(MyDSGroup, all_means_Test)
{
	...
	allMeans = all_means(Data, 310, 17);
...
{
	initialClusterMeans = initia_k_Means(2, allMeans, 310);
	double expectedArray[2] = { 155, 164 };  //means of first two clusters
	for (int i = 0; i < 2; i++)
	{
		EXPECT_DOUBLE_EQ(initialClusterMeans[i], expectedArray[i]);
	}
...
TEST(MyDSGroup, Dist_Test) //testing for only 1 iteration
{
	...
	for (int i = 0; i < 2; i++)
	{
		EXPECT_DOUBLE_EQ(newClusterMeans[i], expectedArray[i]);
	}
...
TEST(MyDSGroup, Prediction_Test1)
{
